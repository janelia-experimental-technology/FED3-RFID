/*
  RFID capabilities have been integrated into this sketch for FEDmodes 0-3
  RFID reader: https://www.amazon.com/LICHIFIT-134-2K-Module-ISO11784-Distance/dp/B0B8NXKPZB
  Transponders: https://www.biomark.com/product/biotherm13-pit-tag/

  Daniel K. Smith
  smithd3@janelia.hhmi.org
  March 1, 2024
...
//configure sercom pins on adafruit feather m0 to read RFID card
Uart Serial2 (&sercom4, 23, 22, SERCOM_RX_PAD_0, UART_TX_PAD_2);
...
void setup() {
  ...
  Serial2.begin(9600);                                 //set RFID sercom communication
  analogWrite (RFID_RST, 0);                           //put RFID card in sleep mode
}
...
void loop() {
  ...
  if (fed3.FEDmode == 0) {
    ...
    fed3.rfidInit();                                    //initialize RFID reader communications
    fed3.rfidRun();                                     //begin RFID tx
    analogWrite (RFID_RST, 0);                          //set RFID reset to low (sleep)           
...
void FED3::rfidInit()
{
    analogWrite (RFID_RST, 255);                        //reset RFID card (wake up)
    Serial2.begin(9600);                                //begin sercom communications
    checksum = false;                                   //set checksums
    invChecksum = false;
    rfidTag = 0;                                        //initialize transponder variables
    rfidTemp = 0.0;
    temp = 0;
    delay(250);                                         //delay to allow reader to begin transmission
}

void FED3::rfidRun()
{   
  while ((Serial2.available()) && ((!checksum) || (!invChecksum)))
  {
    ...
    if (started) // wait for start byte
    {
      buffer[index++] = incomingByte;

      if (incomingByte == 0x03) // end byte
      {
        started = false;
        
        // XOR checksum on bytes 01-26 should match byte 27
        ...
        {
          if(buffer[x] <= '9')
            rfidTag = (rfidTag<<4) + buffer[x] - '0';  
          else
            rfidTag = (rfidTag<<4) + buffer[x] - '7';  
        }

        // Extract the temperature from byte 21.
        
          if(buffer[21] <= '9')
            temp = (temp<<4) + buffer[21] - '0';  
          else
            temp = (temp<<4) + buffer[21] - '7';  

        //Convert byte data to farenheit    
        
        rfidTemp = 74 + ((temp) * 0.2);     

        // Make sure there are no read errors            

        if ((checksum) && (invChecksum))                
        {
        fed3.logRFIDData();                           
        break;
        }
